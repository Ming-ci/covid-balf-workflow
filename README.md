...
### 12 种 Marker 基因 × 10 种细胞类型的表达验证

![Marker基因验证](images/umap_markers.png)

> 每个子图展示一种经典 marker 基因的 UMAP 表达分布（灰色=不表达，红色=高表达），共 12 个基因 × 66,452 个细胞。这些基因共同验证了 10 种细胞类型的注释可靠性：TPPP3/KRT18（上皮）、CD68（巨噬）、FCGR3B（中性粒）、CD1C/CLEC9A（髓系DC）、LILRA4（浆DC）、TPSB2（肥大）、CD3D（T细胞）、KLRD1（NK）、MS4A1（B细胞）、IGHG4（浆细胞）。
